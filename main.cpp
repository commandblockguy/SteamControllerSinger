...
bool SteamController_OpenAndClaim(SteamControllerInfos* controller){
    ...
    if(libusb_kernel_driver_active(controller->dev_handle, controller->interfaceNum)) {
        cout<<"Kernel driver active before"<<endl;
    }
    ...
    if(libusb_kernel_driver_active(controller->dev_handle, controller->interfaceNum)) {
        cout<<"Kernel driver active after"<<endl;
    }
...
int SteamController_PlayNote(SteamControllerInfos* controller, int haptic, int note, double duration, ParamsStruct* params){
    ...
    if(params && params->export_calc) {
        file.write((char*)&note_time, 3);
        file.write((char*)&dataBlob[2], 7);
    }
    num_notes++;
...
void playSong(SteamControllerInfos* controller,const ParamsStruct params){
    ...
    while(currentEvent != NULL){
        ...
        for( ; currentEvent != NULL && MidiFileEvent_getTick(currentEvent) < currentTick ; currentEvent = MidiFileEvent_getNextEventInFile(currentEvent)){
            note_time = MidiFile_getTimeFromTick(midifile, MidiFileEvent_getTick(currentEvent)) * 32768;
        ...
        for(int currentChannel = 0 ; currentChannel < CHANNEL_COUNT ; currentChannel++){
            ...
            //Play notes
            SteamController_PlayNote(controller,currentChannel,eventNote, DURATION_MAX, &params);
...
bool parseArguments(int argc, char** argv, ParamsStruct* params){
    int c;
    while ( (c = getopt(argc, argv, "l:i:r:e")) != -1) {
...
void abortPlaying(int){
    for(int i = 0 ; i < CHANNEL_COUNT ; i++){
        SteamController_PlayNote(&steamController1,i,NOTE_STOP, DURATION_MAX, NULL);
    }
...
int main(int argc, char** argv)
{
    ...
    cout << "Added " << num_notes << " notes." << endl;
